...
class TranslatorClient : WebServiceGatewaySupport() {
    fun translate(translate: TranslationRequest) =
            webServiceTemplate.marshalSendAndReceive(translate) as TranslationResponse
}
...
class Client {
    ...
    @Bean fun marshaller() = Jaxb2Marshaller().apply { contextPath = "client.translator" }

    /** Defines a [TranslatorClient]. */
    @Bean
    fun translatorClient(marshaller: Jaxb2Marshaller) =
            TranslatorClient().apply {
    ...
    fun lookup(translatorClient: TranslatorClient) = CommandLineRunner {
        ...
        try {
            println(
                    "Result of translating [$input] is [${translatorClient.translate(request).translation}]"
            )
